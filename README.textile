...
code with <code><pre>require File.dirname(__FILE__)</pre></code> crap?  What if you could just point something
at a big directory full of code and have everything just automagically load 
regardless of the dependency structure?  Wouldn't that be oh so nice?

Well, now you can!

This gem defines a single method on main:

 <code><pre>load_glob(glob)</code></pre>
...
directory, just do:
 ...
 <code><pre>load_glob 'lib/**/*.rb'</code></pre>
